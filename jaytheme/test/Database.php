<?php
    class Database
    {
        ...
        {
            $this->_conn = new PDO("mysql:host=localhost;dbname=phpdb", "root");
        }
        ...
        {
            $statement = $this->_conn->prepare("UPDATE applications SET $field = :status WHERE ID = :identifier");
            foreach($app_ids as $id) {
                $statement->bindParam(':status', $value);
